...
def build_demo_catalog() -> list[DemoScenarioResponse]:
    return [
        DemoScenarioResponse(
            id="code_debug",
            title="Code Debug Agent Trace",
            description="展示报错分析、工具失败解释、批量评测和审计事件的第一版闭环。",
            capability_focus="代码调试 / 工具失败复盘 / 审计事件",
            default_prompt_version="v2",
        ),
        DemoScenarioResponse(
            id="paper_rag",
            title="Paper / RAG Agent Trace",
            description="展示检索摘要、引用命中和 reference answer judge 的第一版框架。",
            capability_focus="检索问答 / 引用覆盖 / reference answer judge",
            default_prompt_version="v1",
        ),
        DemoScenarioResponse(
            id="robotics_embedded",
            title="Robotics / Embedded Log Analysis",
            description="展示设备日志分析、导航异常定位和人工复核标注的第一版场景。",
            capability_focus="日志分析 / 导航异常定位 / 人工复核",
            default_prompt_version="v2",
        ),
    ]
...
def build_demo_seed_bundles(scenario_id: str) -> list[dict[str, object]]:
    code_debug_bundle = {
        "trace_inputs": [
            {
                "user_input": "Analyze this Python error: TypeError: unsupported operand type(s) for +: 'int' and 'str'",
                "execution_mode": "mock",
                "provider": "deepseek",
            ...
            {
                "user_input": "Summarize a customer support incident timeline and identify the root cause.",
                "execution_mode": "mock",
                "provider": "deepseek",
                "model_name": "deepseek-chat",
                "prompt_version": "v1",
            ...
            {
                "user_input": "Run a safe mock workflow that explains how an AI agent would inspect a failed tool call.",
                "execution_mode": "mock",
                "provider": "deepseek",
                "model_name": "deepseek-chat",
                "prompt_version": "v0",
        ...
        "suite_name": "Code Debug Demo Pack",
        "suite_description": "用于快速演示批量评测、judge 骨架和 trace 评分结果。",
        "cases": [
            {
                "title": "Python Type Error",
                "user_input": "Analyze this Python error: TypeError: unsupported operand type(s) for +: 'int' and 'str'",
                "expected_output": "Likely cause type error int str",
                "ground_truth_type": "keyword",
                "judge_guidance": "检查是否指出类型不匹配和修复方向。",
                "judge_config_json": json.dumps({"keywords": ["type", "error", "int", "str"]}, ensure_ascii=False),
                "score_rubric": "是否指出类型不匹配和修复方向。",
            },
            {
                "title": "Tool Call Failure",
                "user_input": "Explain why a tool call failed and propose the safest next action.",
                "expected_output": "tool failed next action safe",
                "ground_truth_type": "keyword",
                "judge_guidance": "检查是否解释失败原因并给出安全后续动作。",
                "judge_config_json": json.dumps({"keywords": ["tool", "failed", "next", "safe"]}, ensure_ascii=False),
                "score_rubric": "是否解释失败原因并给出安全后续动作。",
            },
        ],
        "prompt_version": "v2",
        "audit_reason": "Demo seed event used to show tool-risk review flow.",
    }
    paper_rag_bundle = {
        "trace_inputs": [
            {
